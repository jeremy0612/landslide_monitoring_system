...
def main(spark, experiment_name, experiment_id, model_name):
    ...
    def filter_by_continent(df, min_longitude, max_longitude, min_latitude, max_latitude, label):
        return df.filter((df.longitude > min_longitude) & 
                        (df.longitude < max_longitude) &
                        (df.latitude < max_latitude) &
                        (df.latitude > min_latitude)) \
                .select(df.temperature.cast("double").alias("temperature"), "rain") \
                .withColumn("label", lit(label))

    # Prepare data for Asia and America with labels
    assembler = VectorAssembler(inputCols=["temperature", "rain"], outputCol="features")
    asia_df = assembler.transform(filter_by_continent(df, 92, 150, -1, 25, 1).withColumn("label", lit(1)))
    america_df = assembler.transform(filter_by_continent(df, -1, 54, -83, 56, 0).withColumn("label", lit(0)))
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        ...
        # mlflow.spark.log_model(spark_model=model, artifact_path="model", registered_model_name=model_name)
